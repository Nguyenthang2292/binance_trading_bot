...
| 5 | **Timeframes** nào để scan? | Nếu user không chắc → suggest `["15m", "30m"]` |
...
| `scan_interval` | `3600` | Rescan mỗi 1 giờ |
| `max_hold_duration` | `86400` | Time-exit sau 24 giờ |

#### 1.3 Hỏi thêm nếu user đề cập (không hỏi mặc định)
...
- scan_interval = 3600s | max_hold = 86400s
- min_confidence = 0.5
...
| `min_notional` | 1.0 | Khớp default hiện tại của engine/SDK |
| `min_confidence` | 0.5 | Filter signal yếu |
| `scan_interval` | 3600s | Hourly rescan |
...
strategy::StrategyConfig parseConfig(const nlohmann::json& j) {
    ...
    cfg.intervals       = j.value("intervals", std::vector<std::string>{"15m", "30m"});
...
| 2 | `plugins/src/<type>/strategy_<type>.cpp` | Tạo mới |
| 3 | `plugins/src/<type>/CMakeLists.txt` | Tạo mới |
| 4 | `config.json` | Thêm entry vào `strategies[]` sau khi DLL tồn tại; nếu chưa có DLL, chỉ xuất snippet chưa áp |
