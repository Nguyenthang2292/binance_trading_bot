...
### 1.1 Global Stop-Loss Gate

`engine.place_stop_loss=false` is an absolute engine-level gate for exchange-side stop-loss placement.

Strategy plugins MUST NOT use `Signal::initialStopPrice`, `exitPolicy`, trailing config, raw params, or any other signal/config field to force `Orders::protection(ProtectionKind::StopLoss)` when the global gate is off.

Allowed:

- Strategy may provide `signal.initialStopPrice` as advisory stop metadata.
- Engine may store advisory stop metadata for trailing/runtime state, such as `currentTrailLevel`.
- Engine may place exchange-side stop-loss orders only when `engine.place_stop_loss=true`.

Forbidden:

- Treating `initialStopPrice > 0` as an override for `engine.place_stop_loss=false`.
- Adding strategy params such as `force_stop_loss`, `always_place_sl`, or similar hidden stop-loss override.
- Calling REST/WebSocket/order APIs from a strategy plugin.

### 1.2 Global Take-Profit Override

`engine.take_profit.enabled=true` makes `engine.take_profit.takeProfitPercent` the authoritative fixed take-profit target for every strategy.

When enabled, engine-level take-profit overrides:

- Strategy `takeProfitPercent`.
- Strategy `tp_multiplier` fallback.
- `Signal::disableFixedTakeProfit`, including Ricardo-style trailing exits.

Allowed:

- Strategy configs may keep their own `takeProfitPercent` for standalone/default behavior.
- Engine must use the global value when the global override is enabled.

Forbidden:

- Letting a strategy-specific `takeProfitPercent=0.0`, `tp_multiplier=0.0`, or `disableFixedTakeProfit=true` suppress global take-profit.
- Adding strategy params such as `force_no_take_profit` that bypass the engine global override.
...
| `engine.place_stop_loss=false` | Strategy stop metadata remains advisory; engine must not place exchange-side SL |
...
- Strategy may set `signal.initialStopPrice` only as advisory stop metadata. It MUST NOT be documented or implemented as a way to force exchange-side SL when `engine.place_stop_loss=false`; engine is the sole authority for stop-loss placement.
...
|---|---|---|---|
| `engine.place_stop_loss=false` is an absolute engine gate | Let `Signal::initialStopPrice` force SL placement | Prevent strategy-specific hidden risk override | Approved |
...
| Strategy stop metadata không được override `engine.place_stop_loss=false` | `Signal::initialStopPrice` chỉ là advisory khi global gate tắt |

### 6.2 Runtime Checklist (SDK Section 4 — agent điền vào design doc)

Trước khi agent tạo design doc, phải verify đủ các điều kiện sau sẽ được implement:
...
| 7 | Strategy-provided stop metadata cannot override `engine.place_stop_loss=false` | Strategy can create exchange-side SL despite global off |
