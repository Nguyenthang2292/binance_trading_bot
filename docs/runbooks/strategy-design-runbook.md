...
| **TP / SL** | `entry ± multiplier × ATR` | Engine (dùng `signal.atr` + config) |
...
| `scan_interval` | `1800` | Rescan mỗi 30 phút — khớp TF thấp nhất (30m) |
...
- `scan_interval_seconds` phải `≤ 1800` khi có `"30m"` trong `intervals` — không được để 3600 vì sẽ bỏ lỡ 1 candle 30m mỗi 2 lần scan.
...
- scan_interval = 1800s | max_hold = 86400s
- min_confidence = 0.5
...
|---|---|---|
| ATR | period=<atr_period> | `signal.atr` — engine dùng cho TP/SL và sizing |
...
| `min_notional` | 1.0 | Khớp default hiện tại của engine/SDK |
| `min_confidence` | 0.5 | Filter signal yếu |
| `scan_interval` | 1800s | Rescan mỗi 30 phút — khớp TF thấp nhất (30m) |
...
strategy::StrategyConfig parseConfig(const nlohmann::json& j) {
    ...
    cfg.scanInterval    = std::chrono::seconds(j.value("scan_interval_seconds", 1800));
...
public:
        ...
        const std::vector<Kline>& klines) const override
    {
        ...
        // ── Tính ATR ──────────────────────────────────────────────────────
        // ATR bắt buộc phải có — engine dùng để tính TP/SL và sizing.
...
|---|---|---|
| ATR | period=<atr_period> | Đo volatility — engine dùng để tính TP/SL và sizing |
| <Indicator 2> | <params> | <mục đích> |
...
| `tp_multiplier` | <tp_multiplier> | TP = entry ± tp × ATR |
...
| 6 | `scan_interval_seconds ≤ 1800` khi `intervals` có `"30m"` | Scan chậm hơn candle → bỏ lỡ signal trên TF thấp nhất |
